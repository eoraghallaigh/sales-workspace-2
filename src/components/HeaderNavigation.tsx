...
export const HeaderNavigation = ({
  ...
  return <header className="fixed top-0 left-0 right-0 w-full h-12 z-50 flex items-center px-3 shrink-0" style={{ background: "#333333" }} onWheel={(e) => e.stopPropagation()}>
      {/* Left section */}
      <div className="flex items-center gap-3 flex-1 min-w-0">
        <Link to={cyclePath("/design-system")} className="p-1.5 rounded-md hover:bg-white/10 transition-colors flex items-center">
      ...
      {/* Center — inspection mode toggle */}
      <button
        type="button"
        onClick={() => window.dispatchEvent(new KeyboardEvent('keydown', { key: 'i', ctrlKey: true, bubbles: true }))}
        className="flex items-center gap-1.5 px-3 py-1 rounded-md border border-white/40 hover:bg-white/20 transition-colors cursor-pointer shrink-0"
      >
        <span className="text-white text-xs font-medium whitespace-nowrap">Inspection Mode (ctrl+i)</span>
      </button>
      ...
      <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
        ...
        <Link to={cyclePath("/outreach-states")} className="p-2 rounded-md hover:bg-white/10 cursor-pointer transition-colors" title="Outreach card states">
